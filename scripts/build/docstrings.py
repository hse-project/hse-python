...
``Transaction`` for information on how deletes within transactions are
handled.
...
``Transaction`` for information on how gets within transactions are handled.
...
overwritten. See ``Transaction`` for information on how puts within
...
``Transaction.commit()`` or``Transaction.abort()`` may be
called.
...
Raises:
    HseException: Underlying C function returned a non-zero value.
""",
    "hse.Transaction.state": """
Retrieve the state of the referenced transaction.

This function is thread safe with different transactions.

Returns:
    TransactionState: Transaction's state.
""",
